...
{
    /// <summary>
    /// Count business days between two dates inclusive, minus closure days.
    /// </summary>
    ...
    /// closureDays are subtracted (whole-month closures like holidays).
    /// </summary>
    public static int MonthBusinessDays(int year, int month, int closureDays = 0)
    {
        var start = new DateTime(year, month, 1);
        var end = new DateTime(year, month, DateTime.DaysInMonth(year, month));
        return Calculate(start, end, closureDays);
    }

    /// <summary>
    /// Business days a voucher was active within a given calendar month.
    /// Clamps to the month boundary so partial starts/ends are handled correctly.
    /// </summary>
